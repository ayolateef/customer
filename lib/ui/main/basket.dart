...
import 'package:courierx/model/basket.dart';
import 'package:courierx/model/dishes.dart';
import 'package:courierx/ui/main/header.dart';
import 'package:courierx/widget/buttonBorder.dart';
import 'package:courierx/widget/icard9a.dart';
...
class _BasketScreenState extends State<BasketScreen>
  ...
  _onItemChangeCount(String id, int value) {
    ...
    setState(() {
      for (var d in basket) if (d.id == id) d.count = value;
    });
  }
        ...
        children: <Widget>[
          ...
          Align(
            ...
            child: Container(
              ...
              child: Column(
                  mainAxisSize: MainAxisSize.min, children: _bottomBar()),
            ),
          ),
        ],
      ),
  ...
  List<Widget> _bottomBar() {
    var list = <Widget>[];
    double _total = 0;
    for (var item in basket) _total += (item.price! * item.count!);

    list.add(_itemText(strings.get(93)!, "\$${_total.toStringAsFixed(2)}",
        false)); // "Subtotal",
    list.add(SizedBox(
      height: 5,
    ));
    ...
    list.add(_itemText(
        strings.get(95)!, "\$${_taxes.toStringAsFixed(2)}", false)); // "Taxes",
    list.add(SizedBox(
      height: 5,
    ));
    _total += _taxes + 30;
    list.add(_itemText(
        strings.get(96)!, "\$${_total.toStringAsFixed(2)}", true)); // "Total",
    list.add(SizedBox(
      height: 15,
    ));
    list.add(Container(
      ...
      child: IButtonBorder(
        ...
        pressButton: _pressCheckoutButton, pressButtonWithId: (int ) {  }, // Checkout
      ),
    ));
    list.add(SizedBox(
      height: 15,
  ...
  List<Widget> _getDataActive() {
    ...
    for (var item in basket) list.add(_item(item));
  ...
  _itemText(String leftText, String rightText, bool bold) {
    ...
    if (bold) _style = theme.text14bold;
    return Container(
